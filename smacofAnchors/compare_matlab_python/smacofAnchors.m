...
function [X,hist] = smacofAnchors(D,X0,W,n_anchors,MDSparam)
...
function [D] = calc_D (X) % distanta euclidia
D = zeros(size(X,1));
for k=1:size(X,1),
    xk = repmat(X(k,:),size(X,1),1); % repmat to subtract one point from all others
