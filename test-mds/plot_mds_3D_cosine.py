...
def plot_mds_3D_cosine(collections, merged_aps):
    ...
    for i in range(len(collections)):
        ...
        for j in range(len(collections)):
            ...
            similarities[i][j] = compare_locations(collections[i], collections[j], cosine, all_aps=merged_aps)
    ...
    plt.title(f'[Cosine][all aps] Reprezentarea a {len(count_floors)} etaje')
    handles = []
    ...
    fig.savefig(f"images/label_mds_3D_cosine_2_floors.svg", bbox_inches='tight')
