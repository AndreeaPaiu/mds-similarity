...
def plot_mds_3D_manhattan(collections, merged_aps):
    ...
    for i in range(len(collections)):
        ...
        for j in range(len(collections)):
            ...
            rssi1, rssi2 = compare_locations(collections[i], collections[j], return_type='rssi', all_aps=merged_aps)
            similarities[i][j] = np.sum(np.abs(np.array(rssi1) - np.array(rssi2)))
    ...
    plt.title(f'[Manhattan][all aps] Reprezentarea a {len(count_floors)} etaje')
    handles = []
    ...
    fig.savefig(f"images/label_mds_3D_manhattan_2_floors.svg", bbox_inches='tight')
